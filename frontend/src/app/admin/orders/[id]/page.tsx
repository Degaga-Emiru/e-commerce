    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '2rem', color: '#f1f5f9', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>
                  {order.userEmail[0].toUpperCase()}
                </div>
                <div>
                  <p style={{ margin: 0, fontWeight: 600 }}>{order.userEmail}</p>
                  <p style={{ margin: 0, fontSize: 12, color: '#94a3b8' }}>User ID: #{order.userId}</p>
                </div>
              </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: '1.5rem' }}>
            <h3 style={{ margin: '0 0 1.25rem', fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
              <CreditCard size={18} color="#6366f1" /> Payment & Logistics
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: '#94a3b8', fontSize: 14 }}>Payment Status</span>
                <span style={{ background: order.paymentStatus === 'COMPLETED' ? '#10b98122' : '#f59e0b22', color: order.paymentStatus === 'COMPLETED' ? '#10b981' : '#f59e0b', padding: '0.2rem 0.6rem', borderRadius: 6, fontSize: 11, fontWeight: 700 }}>
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 13, color: '#cbd5e1' }}>Order Placed: {new Date(order.orderDate).toLocaleString()}</span>
                  </div>
                  {order.shippedDate && (
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: '1.5rem', marginBottom: '2rem' }}>
          <h3 style={{ margin: '0 0 1.5rem', fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
            <ShoppingBag size={18} color="#6366f1" /> Order Items
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.04)' }}>
                <div style={{ display: 'flex', gap: 15, alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: '#94a3b8' }}>ETB {item.unitPrice.toLocaleString()} × {item.quantity}</p>
                  </div>
                </div>
                <p style={{ margin: 0, fontWeight: 700, color: '#fff' }}>ETB {item.totalPrice.toLocaleString()}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ width: '100%', maxWidth: 300, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', color: '#94a3b8' }}>
                <span>Subtotal</span>
                <span>ETB {(order.totalAmount || 0).toLocaleString()}</span>
              </div>
              {order.discountAmount > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#ef4444' }}>
                  <span>Discount</span>
                  <span>-ETB {order.discountAmount.toLocaleString()}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', color: '#94a3b8' }}>
                <span>Shipping</span>
                <span>ETB {(order.shippingAmount || 0).toLocaleString()}</span>
              </div>
