          ...
          {orders.map((order, i) => (
            ...
            >
              ...
              <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                ...
                {/* Shipping & Controls */}
                <div>
                  <h5 style={{ margin: '0 0 1rem', color: '#94a3b8', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Shipping Management</h5>
                  
                  {editingTracking === order.id ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', background: 'rgba(99, 102, 241, 0.05)', padding: '1rem', borderRadius: '16px', border: '1px solid rgba(99, 102, 241, 0.1)' }}>
                      <input 
                        placeholder="Carrier (e.g. DHL)" 
                        value={trackingData.carrier} 
                        onChange={e => setTrackingData({...trackingData, carrier: e.target.value})}
                        style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', padding: '0.75rem', borderRadius: '8px', color: 'white', outline: 'none' }}
                      />
                      <input 
                        ...
                        style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', padding: '0.75rem', borderRadius: '8px', color: 'white', outline: 'none' }}
                      />
                      <button onClick={() => setEditingTracking(null)} style={{ background: '#6366f1', color: 'white', border: 'none', padding: '0.6rem', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>Apply Details</button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.03)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '1rem' }}>
                       <div>
                         <p style={{ margin: 0, fontSize: '0.85rem', color: '#f1f5f9', display: 'flex', alignItems: 'center', gap: '6px' }}>
                           <Truck size={14} color="#6366f1" /> {order.shipping?.carrier || 'Carrier not set'}
                         </p>
                         <p style={{ margin: '0.25rem 0 0', fontSize: '0.8rem', color: '#64748b' }}>Track: {order.shipping?.trackingNumber || 'No tracking ID'}</p>
                       </div>
                       <button onClick={() => {
                          setEditingTracking(order.id);
                          setTrackingData({ carrier: order.shipping?.carrier || '', trackingNumber: order.shipping?.trackingNumber || '', note: '' });
                       }} style={{ background: 'transparent', border: 'none', color: '#6366f1', fontSize: '0.85rem', fontWeight: 700, cursor: 'pointer' }}>
                          Edit
                       </button>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginTop: '1rem' }}>
                    <p style={{ margin: 0, fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>Update Status:</p>
