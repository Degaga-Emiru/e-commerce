...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT oi.product, SUM(oi.quantity) as totalSold, SUM(oi.totalPrice) as totalRevenue " +
           "FROM OrderItem oi " +
           "WHERE oi.sellerOrder.seller.id = :sellerId " +
           "GROUP BY oi.product " +
           "ORDER BY totalSold DESC")
    List<Object[]> findTopSellingProductsBySeller(@Param("sellerId") Long sellerId, Pageable pageable);
}
